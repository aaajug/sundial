defmodule SundialWeb.BoardLive.Sections do
  ...
  def actions(%{assigns: assigns}) do
    ...
    <div style="position: absolute; right: 23px; bottom: 20px; display: grid;">
      <%= live_patch to: Routes.board_index_path(@socket, :edit, %SerialBoard{id: assigns.board["id"]}), id: "edit-board" do %>
      <%= #link to: "/" do %>
        <ion-icon name="pencil-outline" class="is-clickable action-button" style="color:#122329;"></ion-icon>
      <% end %>
      <a id={"delete-board-" <> Integer.to_string(assigns.board["id"])} phx-click="delete" phx-value-id={assigns.board["id"]} data-confirm="This board will be deleted. Are you sure?" phx-target={assigns.myself}>
