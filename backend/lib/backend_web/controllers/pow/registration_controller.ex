defmodule Pow.Phoenix.RegistrationController do
  ...
  def process_create(conn, %{"user" => user_params}) do
    IO.inspect "api-register: in process create"
    Plug.create_user(conn, user_params)
  end

  @spec respond_create({:ok | :error, map(), Conn.t()}) :: Conn.t()
  def respond_create({:ok, _user, conn}) do
    IO.inspect "api-register: in respond create ok"

    text conn, {:ok, "Account created successfully. You may now login."}
    # conn
  ...
  def respond_create({:error, changeset, conn}) do
    IO.inspect "api-register: in respond create error"

    text conn, {:error, "Can't create account right now."}
    # conn
    # |> assign(:changeset, changeset)
    # |> render("new.html")
  end
